  ...
  <div class="w-screen h-screen overflow-auto dark:bg-slate-700 bg-gray-300">
    <modal [(visible)]="modal" [closable]="false">
      <div>
        <form id="token-form">
          <div class="flex">
            <input class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600" 
              [(ngModel)]="token" [ngModelOptions]="{standalone: true}" type="text" placeholder="StorageApi Token">

              <div class="min-w-10 px-2 flex items-center">
                <button type="submit" form="token-form" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center me-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" (click)="sendToken()">
                  @if (!loading) {
...
export class AppComponent implements OnInit {
  ...
  sendToken() {
    this.loading = true;
    this.auth.validateToken(this.token).then(data => {
      console.log(data);
      ...
      if (data.success) {
        ...
        this.tokenMessage = data.message ?? "Invalid token";
      }
    })
    .catch(error => {
      console.log(error);
      
      this.tokenMessage = error.error.message ?? "Invalid token";
    })
